...
Input: n = 2
Output: 8
Explanation: There are 8 records with length 2 that are eligible for
an award:
"PP", "AP", "PA", "LP", "PL", "AL", "LA", "LL"
Only "AA" is not eligible because there are 2 absences (there need to
be fewer than 2).
...
class Solution:
    def checkRecord(self, n: int) -> int:
        MOD = 10**9 + 7
        dp = {(0, 0): 1, (0, 1): 1, (1, 0): 1}
        for _ in range(1, n):
            nxt = {}
            for (absences, lates), value in dp.items():
                nxt[(absences, 0)] = (nxt.get((absences, 0), 0) + value) % MOD
                if absences < 1:
                    nxt[(absences + 1, 0)] = (nxt.get((absences + 1, 0), 0) + value) % MOD
                if lates < 2:
                    nxt[(absences, lates + 1)] = (nxt.get((absences, lates + 1), 0) + value) % MOD
            dp = nxt
