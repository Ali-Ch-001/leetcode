...
press the center button to spell, which also counts as one step. After
the pressing, you could begin to spell the next character in the key
...
Output: 4
Explanation:
For the first key character 'g', since it is already in place, we
just need 1 step to spell this character.
For the second key character 'd', we need to rotate the ring
"godding" anticlockwise by two steps to make it become "ddinggo".
...
class Solution:
    def findRotateSteps(self, ring: str, key: str) -> int:
        ...
        for ch in key:
            next_dp = {}
            for target in positions[ch]:
                best = float("inf")
                for current, steps in dp.items():
                    distance = abs(target - current)
                    best = min(best, steps + min(distance, n - distance))
                next_dp[target] = best
            dp = next_dp
