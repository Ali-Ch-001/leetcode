...
of numbers, possibly duplicates (i.e., a multiset). It should support
inserting and removing specific elements and also reporting a random
element.
...
present. Returns true if the item is present, false otherwise. Note
that if val has multiple occurrences in the multiset, we only remove
one of them.

• int getRandom() Returns a random element from the current multiset
of elements. The probability of each element being returned is
linearly related to the number of the same values the multiset
contains.
...
randomizedCollection.insert(1);   // return true since the collection
does not contain 1.
                                  // Inserts 1 into the collection.
randomizedCollection.insert(1);   // return false since the
collection contains 1.
// Inserts another 1 into the
collection. Collection now contains [1,1].
randomizedCollection.insert(2);   // return true since the collection
does not contain 2.
...
• At most 2 * 10^5 calls in total will be made to insert, remove,
and getRandom.
...
class RandomizedCollection:
    ...
    def remove(self, val: int) -> bool:
        if not self.index[val]:
            return False
        position = self.index[val].pop()
        last = self.values[-1]
        if position != len(self.values) - 1:
            self.values[position] = last
            self.index[last].discard(len(self.values) - 1)
            self.index[last].add(position)
        self.values.pop()
        return True
