"""
363. Max Sum of Rectangle No Larger Than K
Difficulty: Hard
https://leetcode.com/problems/max-sum-of-rectangle-no-larger-than-k/

──────────────────────────────────────────────────

Given an m x n matrix matrix and an integer k, return the max sum of
a rectangle in the matrix such that its sum is no larger than k.

It is guaranteed that there will be a rectangle with a sum no larger
than k.

 

Example 1:

Input: matrix = [[1,0,1],[0,-2,3]], k = 2
Output: 2
Explanation: Because the sum of the blue rectangle [[0, 1], [-2, 3]]
is 2, and 2 is the max number no larger than k (k = 2).

Example 2:

Input: matrix = [[2,2,-1]], k = 3
Output: 3

 

Constraints:

	• m == matrix.length

	• n == matrix[i].length

	• 1 <= m, n <= 100

	• -100 <= matrix[i][j] <= 100

	• -10^5 <= k <= 10^5

 

Follow up: What if the number of rows is much larger than the number
of columns?
"""

import bisect


class Solution:
    def maxSumSubmatrix(self, matrix: list[list[int]], k: int) -> int:
        rows, cols = len(matrix), len(matrix[0])
        best = float("-inf")
        for top in range(rows):
            sums = [0] * cols
            for bottom in range(top, rows):
                for c in range(cols):
                    sums[c] += matrix[bottom][c]
                prefix = [0]
                current = 0
                for value in sums:
                    current += value
                    index = bisect.bisect_left(prefix, current - k)
                    if index < len(prefix):
                        best = max(best, current - prefix[index])
                    bisect.insort(prefix, current)
        return best
