...
Given an integer array machines representing the number of dresses in
each washing machine from left to right on the line, return the
...
Output: -1
Explanation:
It's impossible to make all three washing machines have the same
number of dresses.
...
class Solution:
    def findMinMoves(self, machines: list[int]) -> int:
        ...
        for load in machines:
            running += load - target
            result = max(result, abs(running), load - target)
        return result
