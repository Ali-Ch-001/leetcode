...
represents the j^th query where you must find the answer for Cj / Dj =
?.
...
class Solution:
    def calcEquation(self, equations: List[List[str]], values: List[float], queries: List[List[str]]) -> List[float]:
        ...
        def solve(start: str, end: str, visited: set) -> float:
            ...
            for neighbor, weight in graph[start].items():
                if neighbor not in visited:
                    result = solve(neighbor, end, visited)
                    if result != -1.0:
                        return weight * result
            return -1.0

        return [solve(a, b, set()) for a, b in queries]
