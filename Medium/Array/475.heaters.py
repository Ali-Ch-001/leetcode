...
return the minimum radius standard of heaters so that those heaters
...
Output: 1
Explanation: The two heaters were placed at positions 1 and 4. We
need to use a radius 1 standard, then all the houses can be warmed.
    ...
    def findRadius(self, houses: list[int], heaters: list[int]) -> int:
        ...
        for house in houses:
            ...
            if index < len(heaters):
                distance = min(distance, heaters[index] - house)
            if index > 0:
                distance = min(distance, house - heaters[index - 1])
