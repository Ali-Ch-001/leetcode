...
and subdirectory subsubdir1. subdir2 contains a subdirectory
subsubdir2, which contains a file file2.ext.
...
the file/directory itself, all concatenated by '/'s. Using the above
...
format, return the length of the longest absolute path to a file in
the abstracted file system. If there is no file in the system, return
0.

Note that the testcases are generated such that the file system is
valid and no file or directory name has length 0.
    ...
    def lengthLongestPath(self, input: str) -> int:
        ...
        for line in input.split("\n"):
            ...
            while len(stack) > depth + 1:
                stack.pop()
            if "." in name:
                best = max(best, stack[-1] + len(name))
            else:
