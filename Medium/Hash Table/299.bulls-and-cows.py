...
bulls and y is the number of cows. Note that both secret and guess may
...
the non-bull digits can only be rearranged to allow one 1 to be a
bull.
    ...
    def getHint(self, secret: str, guess: str) -> str:
        ...
        for a, b in zip(secret, guess):
            if a == b:
                ...
                secret_count[a] = secret_count.get(a, 0) + 1
                guess_count[b] = guess_count.get(b, 0) + 1
        cows = sum(min(count, guess_count.get(ch, 0)) for ch, count in secret_count.items())
