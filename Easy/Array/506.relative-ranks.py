...
score of the i^th athlete in a competition. All the scores are
guaranteed to be unique.

The athletes are placed based on their scores, where the 1^st place
athlete has the highest score, the 2^nd place athlete has the 2^nd
...
• For the 4^th place to the n^th place athlete, their rank is their
placement number (i.e., the x^th place athlete's rank is "x").
...
class Solution:
    def findRelativeRanks(self, score: list[int]) -> list[str]:
        ...
        for place, index in enumerate(order):
            result[index] = medals[place] if place < 3 else str(place + 1)
        return result
